...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(HTMLEscapeMap*)table 
  ...
  if (!buffer) {
    // the declaration of *data has been moved to outer scope to avoid beeing recycled by ARC accidentally
    data = [NSMutableData dataWithLength:length * sizeof(UniChar)];
    if (!data) {
    ...
    [self getCharacters:[data mutableBytes] range:NSMakeRange(0, length)];
    buffer = [data bytes];
  }
